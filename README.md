...
Убедитесь, что в корневой папке проекта (на одном уровне с `docker-compose.yml`) лежит файл `.env` с указанием порта:
```text
APP_PORT=8083
```

### 2. Сборка и запуск всех контейнеров
Запустите всю экосистему в фоновом режиме одной командой:
```bash
docker compose up -d --build
```

### 3. Разворачивание таблиц БД
